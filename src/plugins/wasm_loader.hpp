 ...
 *
 * @details
 * This header is private to the `tmm_plugins` library and is not installed.
...
namespace tmm::plugins {
	 ...
	 * (e.g. IDatasetSource instances) registered during tmm_plugin_init.
	 *
	 * Lifecycle:
	 * - Created by wasmLoaderLoad().
	 * - Destroyed by wasmLoaderUnload(), which calls tmm_plugin_teardown and
	 *   releases all WAMR handles.
	 *
	 * @see wasmLoaderLoad
	 * @see wasmLoaderUnload
	 */
	struct Plugin {
		/* -----------------------------------------------------------------
		...
		/**
		 * @brief Persistent copy of tmm_host_api used as WAMR user-data.
		 *
		 * @details
		 ...
		 * is only valid during tmm_plugin_init.
		 */
		 ...
		 * -------------------------------------------------------------- */
		wasm_function_inst_t fnFitBegin = nullptr;		///< @see tmm_on_fit_begin
		wasm_function_inst_t fnEpochBegin = nullptr;	///< @see tmm_on_epoch_begin
		wasm_function_inst_t fnBatchBegin = nullptr;	///< @see tmm_on_batch_begin
		wasm_function_inst_t fnLossComputed = nullptr;	///< @see tmm_on_loss_computed
		wasm_function_inst_t fnBatchEnd = nullptr;		///< @see tmm_on_batch_end
		wasm_function_inst_t fnEpochEnd = nullptr;		///< @see tmm_on_epoch_end
		wasm_function_inst_t fnValidationEnd = nullptr; ///< @see tmm_on_validation_end
		wasm_function_inst_t fnFitEnd = nullptr;		///< @see tmm_on_fit_end
		wasm_function_inst_t fnOnLog = nullptr;			///< @see tmm_on_log
		wasm_function_inst_t fnOnMetric = nullptr;		///< @see tmm_on_metric
		wasm_function_inst_t fnTeardown = nullptr;		///< @see tmm_plugin_teardown
	 ...
	 * (e.g. in unit tests) are safe.  Each successful call to wasmLoaderInit
	 * must be paired with exactly one call to wasmLoaderDestroy.
	 *
	 * @return `{}` on success, or an error string on failure.
	 */
	[[nodiscard]] std::expected<void, std::string> wasmLoaderInit();
	 ...
	 * 3. Register host import functions under the `"tmm"` module namespace.
	 * 4. Instantiate via `wasm_runtime_instantiate()`.
	 * 5. Call `tmm_plugin_get_info`; check ABI version.
	 * 6. Build a #tmm_host_api and call `tmm_plugin_init`.
	 ...
	 * @see wasmLoaderUnload
	 */
	[[nodiscard]] std::expected<void, std::string> wasmLoaderLoad(
	 ...
	 *
	 * @details
	 * Calls `tmm_plugin_teardown` (if exported), then destroys the execution
	...
	wasmPushString(wasm_module_inst_t inst, std::string_view str, uint32_t& wasm_ptr, uint32_t& len);
