...
namespace tmm::plugins {
	...
	std::expected<void, std::string> nativeLoaderLoad(
		...
		if (plugin.dlHandle == nullptr) {
			return std::unexpected(std::format("native_loader_load: cannot open '{}': {}", path.string(), lib_error()));
		}

		/* 2. Resolve required symbols --------------------------------------- */
		plugin.fnGetInfo = resolve<tmm_plugin_info* (*)()>(plugin.dlHandle, "tmm_plugin_get_info");
		if (plugin.fnGetInfo == nullptr) {
			lib_close(plugin.dlHandle);
			plugin.dlHandle = nullptr;
			return std::unexpected(
					std::format("native_loader_load: '{}' does not export tmm_plugin_get_info", path.string())
			);
		}

		plugin.fnInit =
				resolve<tmm_error (*)(const tmm_host_api*, const char*, uint32_t)>(plugin.dlHandle, "tmm_plugin_init");
		if (plugin.fnInit == nullptr) {
			lib_close(plugin.dlHandle);
			plugin.dlHandle = nullptr;
			return std::unexpected(
					std::format("native_loader_load: '{}' does not export tmm_plugin_init", path.string())
			);
		}

		/* 3. Verify ABI version --------------------------------------------- */
		const tmm_plugin_info* info = plugin.fnGetInfo();
		if (info == nullptr) {
			lib_close(plugin.dlHandle);
			plugin.dlHandle = nullptr;
			return std::unexpected("native_loader_load: tmm_plugin_get_info returned null");
		}
		if (info->abiVersion != TMM_ABI_VERSION) {
			...
			return std::unexpected(
					std::format(
							"native_loader_load: plugin ABI version mismatch (plugin={}, host={})", info->abiVersion,
							TMM_ABI_VERSION
					)
			);
		}

		/* 4. Call tmm_plugin_init ------------------------------------------- */
		...
		if (init_result != TMM_OK) {
			...
			plugin.dlHandle = nullptr;
			return std::unexpected(
					std::format("native_loader_load: tmm_plugin_init returned error {}", static_cast<int>(init_result))
			);
		}

		/* 5. Resolve optional lifecycle hooks -------------------------------- */
		plugin.fnTeardown = resolve<void (*)()>(plugin.dlHandle, "tmm_plugin_teardown");
		plugin.fnFitBegin = resolve<void (*)(const char*, uint32_t)>(plugin.dlHandle, "tmm_on_fit_begin");
		plugin.fnEpochBegin = resolve<void (*)(uint32_t, uint32_t)>(plugin.dlHandle, "tmm_on_epoch_begin");
		plugin.fnBatchBegin = resolve<void (*)(uint32_t, uint32_t)>(plugin.dlHandle, "tmm_on_batch_begin");
		plugin.fnLossComputed = resolve<float (*)(float)>(plugin.dlHandle, "tmm_on_loss_computed");
		plugin.fnBatchEnd =
				resolve<void (*)(uint32_t, float, const char*, uint32_t)>(plugin.dlHandle, "tmm_on_batch_end");
		plugin.fnEpochEnd = resolve<int32_t (*)(uint32_t, const char*, uint32_t)>(plugin.dlHandle, "tmm_on_epoch_end");
		plugin.fnValidationEnd = resolve<void (*)(const char*, uint32_t)>(plugin.dlHandle, "tmm_on_validation_end");
		plugin.fnFitEnd = resolve<void (*)(const char*, uint32_t)>(plugin.dlHandle, "tmm_on_fit_end");
		plugin.fnOnLog = resolve<void (*)(uint32_t, const char*, uint32_t)>(plugin.dlHandle, "tmm_on_log");
		plugin.fnOnMetric = resolve<void (*)(const char*, uint32_t, float, int32_t)>(plugin.dlHandle, "tmm_on_metric");
		plugin.fnOnModelLoaded = resolve<void (*)(const char*, uint32_t)>(plugin.dlHandle, "tmm_on_model_loaded");
