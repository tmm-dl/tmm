...
namespace ttm::model {
	...
	public:
		...
		void bind_params(std::vector<ParamBuffer>& params) override {
			...
			loader_->bind_params(handle_,
				pTensors.data(), static_cast<uint32_t>(pTensors.size()),
				gTensors.data(), static_cast<uint32_t>(gTensors.size()));
		}
		...
		void zero_grad() override {
			loader_->zero_grad(handle_);
		}

		// Direct tensor-based call paths used by ModelPipeline ──────────────

		[[nodiscard]] trainer::StepOutput step_with_tensors(
		...
		[[nodiscard]] ttm_error describe_params(
			const ttm_param_desc_t** out_descs, uint32_t* out_count
		) {
			return loader_->describe_params(handle_, out_descs, out_count);
		}

		[[nodiscard]] ttm_error init_params(std::string_view method) {
			return loader_->init_params(handle_, method);
		}
	...
	namespace {
		...
		ModelInfo from_c_info(const ttm_model_info_t& ci, Device dev) {
			ModelInfo m;
			if (ci.name)             m.name             = ci.name;
			if (ci.arch)             m.arch             = ci.arch;
			m.num_parameters  = ci.num_parameters;
			m.num_trainable   = ci.num_trainable;
			m.bytes_on_device = ci.bytes_on_device;
			if (ci.input_schema_json) m.input_schema_json = ci.input_schema_json;
			m.device = dev;
			return m;
		}
		...
		std::string build_model_info_json(const ModelInfo& m) {
			return std::format(
				...
				m.name, m.arch,
				m.num_parameters, m.num_trainable, m.bytes_on_device,
				[&m]() -> std::string {
	...
	std::expected<std::unique_ptr<ModelPipeline>, std::string>
	ModelPipeline::load(
		const conf::ModelConfig&                     cfg,
		const std::vector<conf::PreprocessorEntry>&  preprocessor_entries,
		...
		if (!openResult) {
			return std::unexpected(std::format(
				"ModelPipeline: failed to load '{}': {}", cfg.path, openResult.error()
			));
		}
		...
		if (info.name.empty()) info.name = cfg.path;
		...
		if (desc_err == TTM_OK && descs != nullptr && n_descs > 0) {
			...
			for (uint32_t i = 0; i < n_descs; ++i) {
				...
				total_bytes += nbytes;
				if (params[i].trainable) ++trainable_count;
				...
				tensor.dtype       = {static_cast<uint8_t>(desc.dtype_code),
				                      static_cast<uint8_t>(desc.dtype_bits), 1};
	...
	std::string_view ModelPipeline::name() const {
		return info_.name;
	}
	...
	void ModelPipeline::bind_params(std::vector<ParamBuffer>& params) {
		if (inner_) inner_->bind_params(params);
	}
