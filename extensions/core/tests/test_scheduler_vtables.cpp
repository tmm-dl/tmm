...
std::string make_cfg(int64_t warmup = 0, float min_lr = 0.0f,
                     int64_t step_size = 1, float gamma = 0.1f,
                     int64_t total_steps = 0)
{
    ...
    std::snprintf(buf, sizeof(buf),
        ...
        static_cast<double>(min_lr),
        static_cast<long long>(step_size),
...
TEST_CASE("cosine_warmup with zero warmup acts like plain cosine", "[core][scheduler]") {
    ...
    for (int64_t step : {0, 1, 50, 100, 150, 200}) {
        INFO("step=" << step);
        CHECK_THAT(vt_cw.step(h_cw, step),
                   Catch::Matchers::WithinAbs(vt_c.step(h_c, step), kEps));
    }
