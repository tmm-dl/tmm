...
namespace {
	...
	tmm_handle constantCreate(float base_lr, const char* cfg, uint32_t cfg_len) {
		return allocSched(parseSchedCfg(base_lr, cfg, cfg_len));
	}

	float constantStep(tmm_handle h, [[maybe_unused]] int64_t global_step) {
...
tmm_scheduler_vtable g_sched_constant = {constantCreate, constantStep, schedDestroy};
