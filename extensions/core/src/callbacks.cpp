...
static float metrics_get(std::string_view metrics_json, std::string_view key) {
	...
	if (s.empty() || s == "null") return std::numeric_limits<float>::quiet_NaN();
...
static EarlyStoppingState* es_get(ttm_handle h) {
	if (h < 0 || h >= kMaxSlots || !s_es_used[static_cast<int>(h)]) return nullptr;
...
static void early_stopping_on_fit_begin(ttm_handle h, const char* /*metrics*/, uint32_t /*len*/) {
	...
	st->best    = st->mode_min ? std::numeric_limits<float>::infinity()
	                           : -std::numeric_limits<float>::infinity();
	st->wait    = 0;
	st->stopped = false;
}

static int32_t early_stopping_on_epoch_end(ttm_handle h, int64_t /*epoch*/,
                                            const char* metrics_json, uint32_t len) {
	auto* st = es_get(h);
	if (st == nullptr || st->stopped) return (st != nullptr && st->stopped) ? 1 : 0;

	const std::string_view mj{metrics_json, len};
	const float current = metrics_get(mj, st->monitor);
	if (std::isnan(current)) return 0; // metric not yet available — don't stop

	const bool improved = st->mode_min
		? (current < st->best - st->min_delta)
		: (current > st->best + st->min_delta);
...
static void early_stopping_destroy(ttm_handle h) {
	if (h >= 0 && h < kMaxSlots) s_es_used[static_cast<int>(h)] = false;
}
...
static CheckpointState* ck_get(ttm_handle h) {
	if (h < 0 || h >= kMaxSlots || !s_ck_used[static_cast<int>(h)]) return nullptr;
...
static ttm_handle checkpoint_create(const char* cfg, uint32_t cfg_len) {
	...
	if (dir.empty()) dir = json_get_str(json, "dir", "checkpoints");
	...
	if (n < 0) n = json_get_int(json, "save_every_n_epochs", 1);
...
static int32_t checkpoint_on_epoch_end(ttm_handle h, int64_t epoch,
                                        const char* metrics_json, uint32_t len) {
	auto* st = ck_get(h);
	if (st == nullptr) return 0;
	++st->epoch_count;
	if (st->epoch_count % st->every_n_epochs != 0) return 0;
...
static void checkpoint_destroy(ttm_handle h) {
	if (h >= 0 && h < kMaxSlots) s_ck_used[static_cast<int>(h)] = false;
}
...
void callbacksRegister(const ttm_host_api* host) {
	if (host == nullptr || host->register_callback == nullptr) return;
