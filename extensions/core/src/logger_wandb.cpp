...
static WandbState* wbGet(ttm_handle h) {
	if (h < 0 || h >= kMaxWandbSlots || !s_wb_used[static_cast<int>(h)]) return nullptr;
...
static int32_t wandbOnEpochEnd(ttm_handle h, int64_t epoch,
                                const char* metricsJson, uint32_t len) {
	auto* st = wbGet(h);
	if (st == nullptr || st->histFile == nullptr) return 0;
	st->step = epoch + 1;
	...
	if (pairs.empty()) {
		...
		std::fprintf(st->histFile, "{%s,\"_step\":%lld,\"_timestamp\":%.3f}\n",
		             pairs.c_str(), static_cast<long long>(st->step), ts);
	}
...
static void wandbDestroy(ttm_handle h) {
	...
	if (h >= 0 && h < kMaxWandbSlots) s_wb_used[static_cast<int>(h)] = false;
}
