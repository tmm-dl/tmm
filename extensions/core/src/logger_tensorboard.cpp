...
namespace {
	...
	static void
	jsonForEachMetric(std::string_view json, void (*fn)(std::string_view key, float val, void* ud), void* ud) {
		auto pos = json.find('{');
		if (pos == std::string_view::npos)
			return;
		...
		while (!json.empty()) {
			while (!json.empty() &&
				   (json.front() == ' ' || json.front() == '\t' || json.front() == '\n' || json.front() == ','))
	...
	static void pbBytesField(std::vector<uint8_t>& buf, int fieldNum, const std::vector<uint8_t>& data) {
	...
	static bool tfWriteRecord(FILE* f, const std::vector<uint8_t>& data) {
		...
		if (std::fwrite(&len, 8, 1, f) != 1)
			return false;
		if (std::fwrite(&lenCrc, 4, 1, f) != 1)
			return false;
		if (std::fwrite(data.data(), 1, data.size(), f) != data.size())
			return false;
		if (std::fwrite(&dataCrc, 4, 1, f) != 1)
			return false;
		return true;
	}
	...
	static std::vector<uint8_t> tfVersionEvent(double wallTime) {
		...
		uint64_t wt = 0;
		std::memcpy(&wt, &wallTime, 8);
		pbFixed64Field(ev, 1, wt);
		pbStringField(ev, 7, "brain.Event:2");
		return ev;
	}

	static std::vector<uint8_t> tfScalarEvent(double wallTime, int64_t step, std::string_view tag, float value) {
	...
	static void tensorBoardOnFitBegin(ttm_handle h, const char* /*metrics*/, uint32_t /*len*/) {
		...
		const std::string fname = std::string(st->logDir) + "/events.out.tfevents." + std::to_string(ts);
		st->eventsFile = std::fopen(fname.c_str(), "wb");
		if (st->eventsFile == nullptr)
			return;
	...
	static void tensorBoardDestroy(ttm_handle h) {
		...
		if (h >= 0 && h < kMaxTbSlots)
			s_tb_used[static_cast<int>(h)] = false;
	}
