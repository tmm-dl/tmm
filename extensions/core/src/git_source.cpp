...
namespace {
	 ...
	 */
	std::filesystem::path
	gitEnsureRepo(const std::string& git_url, const std::string& ref, char* err, uint32_t err_cap) {
...
#ifdef _WIN32
		if (_fseeki64(fp, static_cast<__int64>(offset), posixWhence) != 0)
			return -1;
		return static_cast<int64_t>(_ftelli64(fp));
#else
		if (fseeko(fp, static_cast<off_t>(offset), posixWhence) != 0)
			return -1;
