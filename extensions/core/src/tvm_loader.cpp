...
namespace {
	...
	ttm_error tvmDescribeParams(ttm_handle /*h*/, const ttm_param_desc_t** out_descs, uint32_t* out_count) {
		if (out_descs)
			*out_descs = nullptr;
		if (out_count)
			*out_count = 0;
	...
	ttm_error tvmInitParams(ttm_handle /*h*/, const char* /*method*/, uint32_t /*len*/) { return TTM_ERR_UNSUPPORTED; }

	ttm_error tvmStep(ttm_handle /*h*/, const DLTensor* /*inputs*/, uint32_t /*n*/, float* out_loss) {
		if (out_loss)
			*out_loss = 0.0f;
		return TTM_ERR_UNSUPPORTED;
	}
	...
	ttm_error tvmInfer(
			ttm_handle /*h*/, const DLTensor* /*inputs*/, uint32_t /*in_count*/, DLTensor* /*outputs*/,
			uint32_t* out_count
	) {
		if (out_count)
			*out_count = 0;
	...
	ttm_error tvmDescribeParams(ttm_handle /*h*/, const ttm_param_desc_t** out, uint32_t* cnt) {
		if (out)
			*out = nullptr;
		if (cnt)
			*cnt = 0;
	...
	ttm_error
	tvmBindParams(ttm_handle /*h*/, const DLTensor* /*p*/, uint32_t /*pc*/, const DLTensor* /*g*/, uint32_t /*gc*/) {
		return TTM_ERR_UNSUPPORTED;
	}

	ttm_error tvmInitParams(ttm_handle /*h*/, const char* /*m*/, uint32_t /*l*/) { return TTM_ERR_UNSUPPORTED; }

	ttm_error tvmStep(ttm_handle /*h*/, const DLTensor* /*in*/, uint32_t /*n*/, float* loss) {
		if (loss)
			*loss = 0.0f;
		return TTM_ERR_UNSUPPORTED;
	}

	ttm_error tvmInfer(ttm_handle /*h*/, const DLTensor* /*in*/, uint32_t /*ic*/, DLTensor* /*out*/, uint32_t* oc) {
		if (oc)
			*oc = 0;
		return TTM_ERR_UNSUPPORTED;
	}
	...
	ttm_model_loader_vtable g_tvm_loader = {tvmProbe,	   tvmLoad, tvmGetInfo, tvmDescribeParams, tvmBindParams,
