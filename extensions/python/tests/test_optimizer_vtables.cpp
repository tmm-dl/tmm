...
class MinimalModel(torch.nn.Module):
	...
	std::string make_opt_cfg(float lr = 1e-3f, float weight_decay = 0.0f, const char* device = "cpu") {
		char buf[256];
		std::snprintf(
				buf, sizeof(buf), "{\"lr\":%g,\"weight_decay\":%g,\"device\":\"%s\"}", static_cast<double>(lr),
				static_cast<double>(weight_decay), device
		);
		return buf;
	}
...
TEST_CASE("Python plugin loads and registers adamw optimizer", "[python][optimizer]") {
	...
	SECTION("adamw vtable is registered") { CHECK(p.optimizers.count("adamw") == 1); }
